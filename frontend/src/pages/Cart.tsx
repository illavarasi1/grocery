...
import { dummyAddress } from "../assets/assets/assets"
import type { Product } from "../components/types/product"
import refresh from "../assets/assets/remove_icon.svg"
import arrow from "../assets/assets/arrow_right_icon_colored.svg"
        ...
        <div className="flex flex-col md:flex-row mt-[90px]">
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button onClick={() => removeFromCart(product._id)} className="cursor-pointer mx-auto">
                            ...
                            <img src={refresh} alt="remove" className="inline-block w-6 h-6" />
                        </button>
                    </div>)
                )}

                <button onClick={() => { navigate('/products'); scrollTo(0, 0) }} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <img className="group-hover:-translate-x-1 transition" src={arrow} alt="arrow" />
                    Continue Shopping
                </button>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <button onClick={placeorder} className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium hover:bg-primary-dull transition">
                    {paymentOption === "COD" ? "palce order " : "proceed to checkout"}
                </button>
            </div>
        </div>
    ) : null
